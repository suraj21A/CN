...
FREIGHT_FILE = os.path.join(TOOL_DIR, "PUNE REVISED RATE 01.10.2025.xlsx")
...
def is_ev_vehicle(description):
    ...
    ev_keywords = [
        ".EV", " EV ", "NEXON.EV", "TIAGO EV",
        "PUNCH.EV", "HARRIER.EV", "CURVV.EV", "XPRES-T EV",
    ]
...
def get_freight_bucket(mm_material):
    ...
    if material in ["HARRIER", "HARRIER EV", "SAFARI", "CURVV", "CURVV EV"]:
        return "HARRIER/SAFARI/CURVV"
    if material in ["NEXON", "NEXON EV", "ALTROZ"]:
        return "NEXON/ALTROZ"
    if material in ["PUNCH", "PUNCH EV"]:
        return "PUNCH"
    # Pune master doesn't have TIAGO/TIAGOR/SIERRA columns,
    # so unmatched material will get rate 1
    return None
...
def prepare_freight_df(master_path):
    df = pd.read_excel(master_path, sheet_name="Sheet2", header=1)
    df.columns = [str(c).strip() for c in df.columns]
    df = df.dropna(how="all").reset_index(drop=True)
    required_cols = [
        "LOCATION",
        "HARRIER/SAFARI/CURVV",
        "NEXON/ALTROZ",
        "PUNCH",
    ]
    missing = [c for c in required_cols if c not in df.columns]
    ...
    df["DEST_KEY"] = df["LOCATION"].apply(clean_key)
    return df

# ============ NEW: Fuzzy Freight Location Matching ============
def get_rate_from_master(destination, mm_material, freight_df):
    ...
    for idx, row in freight_df.iterrows():
        sim = place_similarity(destination, row["LOCATION"])
...
def process_tata_cn(input_file, filter_date=None):
    print("=" * 60)
    print("TATA PUNE CN PROCESS STARTED")
    print("=" * 60)
    ...
    if filter_date:
        df["parsed_march_date"] = pd.to_datetime(df["March Date"], errors="coerce", dayfirst=True)
    ...
    for _, row in df.iterrows():
        ...
        route = f"Pune-{final_destination}-{NON_EV_CONSIGNOR}"
        rate = get_rate_from_master(final_destination, mm_material, freight_df)
        # ---------- Fuzzy freight rate lookup ----------
        rate = get_rate_from_master(final_destination, mm_material, freight_df)
        freight = rate
        print("-" * 60)
    ...
    output_file = f"TATA_PUNE_CN_{run_datetime}.xlsx"
    wb.save(output_file)
