...
def get_office_by_cn(cn_no):
    """
    Get FIXED_OFFICE based on first digit of CN number
    
    Rules:
    - CN starts with 3 → GURGAON
    - CN starts with 4 → MANESAR
    - Rest → BANGALORE
    """
    if not cn_no:
        return "BANGALORE"
    
    first_digit = str(cn_no).strip()[0]
...
def process_maruti_cn(input_file, filter_date=None):
    ...
    """
    print("=" * 60)
    print("MARUTI CN PROCESS STARTED")
    print("=" * 60)
    ...
    if filter_date:
        df["_invoice_date_parsed"] = pd.to_datetime(
            df["Invoice Date"], errors="coerce", dayfirst=False
        )
    ...
    for _, row in df.iterrows():
        ...
        # ✅ DYNAMIC OFFICE (CN basis pe)
        dynamic_office = get_office_by_cn(cn_no)
        
        # ✅ FIXED BILLING OFFICE (ALWAYS JAIPUR)
        billing_office_fixed = FIXED_BILLING_OFFICE
        ...
        # ✅ BUILD ROUTE WITH DYNAMIC OFFICE
        route = f"{dynamic_office}-{final_destination}-{CONSIGNOR}"

        # Print processing details
        print("-" * 60)
        print(f"Excel Row {excel_row}")
        print("Trip No:", trip_no)
        print("CN First Digit:", str(cn_no).strip()[0])
        print("Office Selected:", dynamic_office)
        print("Billing Office:", billing_office_fixed)
        ...
        # ✅ WRITE DATA TO EXCEL CELLS WITH DYNAMIC OFFICE
        ws.cell(excel_row, 1).value = cn_no
        ws.cell(excel_row, 2).value = FIXED_CN_TYPE
        ws.cell(excel_row, 3).value = invoice_date
        ws.cell(excel_row, 4).value = dynamic_office          # ← DYNAMIC (3=GURGAON, 4=MANESAR, rest=BANGALORE)
        ws.cell(excel_row, 5).value = billing_office_fixed    # ← FIXED (ALWAYS JAIPUR)
    ...
    output_file = f"Maruti_GM_CN_{run_datetime}.xlsx"
